...
Works best with the preconfigured devcontainer setup in vscode and uv installed
...
To setup your project clone from GH and then run the following:
...
- Run all linting and testing. It is worth looking inside this build script as it covers all common dev commands
  ```bash
  ./build.sh
  ```
...
```markdown
::: prefect_managedfiletransfer.{module_name}
```
...
Upon publishing, a `docs` branch is automatically created. To hook this up to GitHub Pages, simply head over to https://github.com/ImperialCollegeLondon/prefect-managedfiletransfer/settings/pages, select `docs` under the dropdown menu, keep the default `/root` folder, `Save`, and upon refresh, you should see a prompt stating "Your site is published at https://ImperialCollegeLondon.github.io/prefect-managedfiletransfer". Don't forget to add this link to the repo's "About" section, under "Website" so users can access the docs easily.
